...
function TerminalPanel() {
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setVisibleCount((count) => Math.min(count + 1, history.length))
    }, visibleCount < 3 ? 180 : 85)
  ...
  const executeCommand = (rawCommand: string) => {
    ...
    setVisibleCount(nextHistory.length)
    setInputValue("")
  ...
  return (
    <div
      ...
      className="card-surface relative overflow-hidden rounded-lg border border-black/10 bg-[var(--color-terminal-bg)] text-[var(--color-terminal-text)] shadow-[0_22px_60px_rgba(15,23,42,0.12)] dark:border-white/[0.08] dark:shadow-[0_22px_60px_rgba(0,0,0,0.35)] cursor-text"
    >
      <div className="flex items-center justify-between gap-4 border-b border-black/10 bg-[var(--color-terminal-chrome)] px-3 py-2.5 select-none dark:border-white/[0.06]">
        <div className="flex items-center gap-2">
          <div className="flex h-5 w-5 items-center justify-center rounded border border-black/15 bg-black/10 text-[var(--color-terminal-text)] dark:border-white/15 dark:bg-black/40">
            <span className="font-mono text-[10px] leading-none">C:\</span>
          </div>
          <span className="font-mono text-[11px] font-medium text-[var(--color-terminal-text)]/90">
            Command Prompt
          </span>
        </div>
        <div className="mr-1 flex items-center gap-3 text-[var(--color-terminal-muted)]">
          {/* Minimize */}
      ...
      <div className="border-b border-black/8 px-4 py-2 font-mono text-[10px] uppercase tracking-[0.22em] text-[var(--color-terminal-muted)] dark:border-white/[0.04]">
        Type <span className="text-[var(--color-terminal-text)]/90">HELP</span> to list available commands
      </div>

      <div className="absolute inset-x-0 top-[73px] h-px bg-black/8 dark:bg-white/5" />
      <div
        className="pointer-events-none absolute inset-0 opacity-[0.04] dark:opacity-[0.03]"
        aria-hidden="true"
        style={{
          backgroundImage:
            "linear-gradient(color-mix(in oklab, var(--color-terminal-text) 22%, transparent) 1px, transparent 1px)",
      ...
      <div
        ref={containerRef}
        className="relative h-[290px] overflow-y-auto bg-transparent px-4 py-4 font-mono text-[15px] leading-[1.35] text-[var(--color-terminal-text)]/90 scrollbar-none"
      >
        {history.slice(0, visibleCount).map((line, index) => (
          <div key={index} className="mb-1.5">
            {line.type === "cmd" ? (
              <div className="flex gap-2">
                <span className="select-none text-[var(--color-terminal-accent)]">&gt;</span>
                <span className="font-semibold text-[var(--color-terminal-text)]/95">{line.text}</span>
              </div>
            ) : line.type === "blank" ? (
              <div className="h-2" />
            ) : line.type === "error" ? (
              <div className="text-[var(--color-terminal-text)]/74">
                <span className="text-[var(--color-terminal-muted)]">C:\</span> {line.text}
              </div>
            ) : (
              <div className="whitespace-pre-line text-[var(--color-terminal-text)]/78">
                {line.text}
              </div>
            )}
          </div>
        ...
        <form onSubmit={handleCommand} className="flex items-start gap-1 pt-2">
          <div className="select-none text-[var(--color-terminal-accent)]">&gt;</div>
          <div className="relative flex min-h-[1.25rem] flex-1 items-center">
            <span className="select-none whitespace-pre text-[var(--color-terminal-text)]/90">
              {inputValue || <span className="text-[var(--color-terminal-muted)]/50">type here...</span>}
            </span>
            <span className="ml-1 inline-block h-[1.1em] w-[0.65ch] bg-[var(--color-terminal-text)]/82" />
            <input
              ref={inputRef}
              type="text"
              value={inputValue}
        ...
        <div className="mt-4 flex flex-wrap gap-2 border-t border-black/8 pt-4 text-[10px] uppercase tracking-[0.18em] text-[var(--color-terminal-muted)] dark:border-white/[0.06]">
          {["help", "projects", "skills", "email", "clear"].map((command) => (
            <button
              key={command}
              type="button"
              onClick={() => executeCommand(command)}
              className="rounded-full border border-black/10 bg-black/[0.03] px-3 py-1.5 font-mono text-[var(--color-terminal-text)]/72 transition-colors hover:bg-black/[0.06] hover:text-[var(--color-terminal-text)] dark:border-white/[0.08] dark:bg-white/[0.03] dark:hover:bg-white/[0.08]"
            >
              {command}
            </button>
          ))}
